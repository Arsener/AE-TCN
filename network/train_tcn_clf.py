...
def create_model(file):
    ...
    with open(
            os.path.join(
                args.save_path, 'tcn_clf_hyperparameters.json'
            ), 'w'
    ) as fp:
        json.dump(params, fp)
...
def parse_arguments():
    ...
    parser.add_argument('--hyper', type=str, metavar='FILE',
                        default='tcn_clf_hyperparameters.json',
...
#     train_dataset = LabelledDataset(train_X)
    ...
    early_stopping = EarlyStopping(params['early_stopping'], path=os.path.join(args.save_path, '{}_TCN_CLF.pth'.format(dataset)))
    ...
    while epoch < params['epochs']:
        ...
        with torch.no_grad():
            ...
            tr_l = criterion(train_pred, train_label).item()
            te_l = criterion(test_pred, test_label).item()
            train_loss.append(tr_l)
            test_loss.append(te_l)
            early_stopping(te_l, model)
            
            tr_acc = acc(train_label.cpu(), train_pred.cpu())
            te_acc = acc(test_label.cpu(), test_pred.cpu())
